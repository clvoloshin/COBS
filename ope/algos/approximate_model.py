...
class ApproxModel(DirectMethodModelBased):
    ...
    def Q(self, policy, x, gamma, t=0):
        ...
        new_state, cost_holder, new_done = self.transition(state, np.atleast_2d(np.eye(self.n_actions)[acts]))
...
class ApproxModel2(object):
    """Algorithm: Approx Model (Model-Based). 

    This is class builds a model from which Q can be estimated through rollouts.
    """
    def __init__(self, gamma, filename, max_traj_length=200, frameskip=2, frameheight=2, processor=None, action_space_dim=3):
        """
        Parameters
        ----------
        gamma : float
        ...
        max_traj_length: int, optional
            How many steps are allowed before episode is over.
            Default: 200
        frameskip : int, optional.
            How many times to apply the same action between transitions
            Default: 2
        frameheight : int, optional.
            How many states to stack on one-another to represent a single state.
            Default: 2
    ...
    def create_full_model(self, input_size):
        ...
        -------
        obj1, obj2
            obj1: Compiled model. (s,a) -> (s', r, done)
            obj2: Forward model. (s) -> ({P(s,a)}_{a \in A}, {R(s,a)}_{a \in A}, {done(s,a)}_{a \in A})
        """
    ...
    def run(self, env, dataset, num_epochs=100, batchsize=32, modeltype='conv'):
        """(Linear/Neural) Get the Model-Based OPE model for pi_e.
